...
file = ROOT.TFile("simoutput.root", "READ")

histograms = [
    "simh_muon_pt", "simh_muon_eta", "simh_muon_phi", "simh_muon_energy", 
    "simh_muon_mass", "simh_muon_leading", "simh_muon_subleading", 
...
for hist_name in histograms:
    ...
    if hist_name == "simh_muon_pt":
    ...
    elif hist_name == "simh_muon_energy":
    ...
    elif hist_name == "simh_muon_leading":
        hist.SetXTitle("p_{T#mu} (GeV)")
        hist.SetYTitle("Entries")
        canvas.SetLogy()
        hist.SetMinimum(1)
    ...
    elif hist_name == "simh_muon_subleading":
        hist.SetXTitle("p_{T#mu} (GeV)")
        hist.SetYTitle("Entries")
        canvas.SetLogy()
        hist.SetMinimum(1)
        ...
        simh_Z_mass_rebinned = ROOT.TH1D("simh_Z_mass_rebinned", "Z boson mass;M_{#mu#mu} (GeV);Entries", n_bins_z_mass, array('d', z_mass_bin_edges))
        for bin_idx in range(1, hist.GetNbinsX() + 1):
            ...
            simh_Z_mass_rebinned.Fill(bin_center, bin_content)
            simh_Z_mass_rebinned.SetBinError(simh_Z_mass_rebinned.FindBin(bin_center), bin_error)
        simh_Z_mass_rebinned.SetLineColor(ROOT.kBlue)
        simh_Z_mass_rebinned.Draw("HIST")
        simh_Z_mass_rebinned.Write()
        canvas.SaveAs(f"{hist_name}_variable_bins.png")

    elif hist_name == "simh_Z_mass_fine":
        hist.SetXTitle("M_{#mu#mu} (GeV)")
        hist.SetYTitle("Entries")
        canvas.SetLogy()
        hist.SetTitle("Z boson mass")

...
class MiniAnalyzer : public edm::one::EDAnalyzer<edm::one::SharedResources> {
   ...
   private:
      ...
      //energy values for muons in the event
...

{
   ...
   h_Z_mass = new TH1D("h_Z_mass", "Z Boson MASS", 1000, 30, 1000);
   h_Z_mass_fine = new TH1D("h_Z_mass_fine", "Z BOSON MASS", 150, 70, 110);
   h_Z_mass_bw = new TH1D("h_Z_mass_bw", "Z BOSON MASS", 150, 70, 110);
}
...
MiniAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   if(muons.isValid() && muons->size() > 2){
      ...
      if (muon1 && muon2){
         ...
         // Calculate properties for each muon
         double pt1 = muon1->pt();
         ...
         if (pt1>=20 && pt2>=12) {
            ...
            // Print out properties of each muon in the pair
            ...
            // Calculate combined properties of the muon pair
            ...
            //double Zboson_mass = sqrt(pow(Zboson_energy,2)-pow(Zboson_pt,2));
...
{
   fs = new TFile("output.root","RECREATE");
}
